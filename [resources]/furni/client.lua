...
blacklistedModels = {
	...
	[12] = -1927046451,
	[13] = -218386588,
	[14] = GetHashKey("playerhouse_tier1"),
	[15] = GetHashKey("V_16_DT"),
	[16] = GetHashKey("playerhouse_tier3"),
...
furni.awake = function()
	...
	while not BJCore.Functions.IsPlayerLoaded()  do Citizen.Wait(0); end

	furni.plyData =  BJCore.Functions.GetPlayerData()
	furni.update     (false)
end
				...
				if furni.editing then
					...
					if ent and ent ~= -1 and ent ~= 0 and DoesEntityExist(ent) then
						...
						if ePos.x ~= 0.0 then
							...
							if not furni.isBlacklisted(model) then
							--if model ~= -860235009 and model ~= -1940280087 and model ~= 2354687209 and model ~= 420356015 then
								if hashtable[model] and hashtable[model].name then
									if wasPressing and keyHeldFor >= 500 then
										BJCore.Functions.DrawText3D(pos.x,pos.y,pos.z,"~r~Release [~s~LMB/RT~r~] to change [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~r~]~s~",1,7)
									else
										BJCore.Functions.DrawText3D(pos.x,pos.y,pos.z,"~y~Hold [~s~LMB/RT~y~] to change [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~y~]~s~",1,7)
									end
					...
					if IsDisabledControlJustPressed(0, Controls["Focus"]) or IsControlJustPressed(0, Controls["Focus"]) then
						TriggerEvent("forceCrosshair", false)
						furni.editing = false
				...
				elseif furni.doingRemoval then
					...
					if ent and ent ~= -1 and ent ~= 0 then
						...
						if ePos and ePos.x and ePos.x ~= 0 and ePos.x ~= 0.0 then
							...
							--if model ~= -860235009 and model ~= -1940280087 and model ~= 2354687209 and model ~= 420356015 then
								if hashtable[model] and hashtable[model].name then
									BJCore.Functions.DrawText3D(pos.x,pos.y,pos.z,"~y~Hold [~s~LMB/RT~y~] to sell [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~y~] [~s~"..(hashtable[model] and "$"..math.floor(hashtable[model].price*(Config.ResaleValue and Config.ResaleValue/100.0 or 0.5)) or "Unknown").."~y~]~s~",1,7)
									if IsDisabledControlPressed(0, Controls["Grab"]) or IsControlPressed(0, Controls["Grab"]) and not furni.focused then
										keyHeldFor = keyHeldFor + (GetGameTimer() - lastFrame)
										if keyHeldFor >= 500 then
											keyHeldFor = 0
...
furni.isBlacklisted = function(model)
    local blacklisted = false
    for i=1, #blacklistedModels, 1 do
	    if model == blacklistedModels[i] then
			blacklisted = true
			break
	    end
    end
...
print("eddint")
	TriggerEvent("forceCrosshair", true)
	furni.editing = true
	furni.overlay("edit")
	furni.focus(false)
end
...
utils.event(true,furni.enterHouse,'furni:client:enteredHouse')
utils.event(true,furni.leaveHouse,'furni:client:leaveHouse')
utils.event(false,furni.toggleFurni,'furni:client:toggleFurni')
...
furni.openCommand = function(s,args) 
	if furni.inHouse then
		...
		BJCore.Functions.Notify("You must be inside a house", "error")
	end
end
