...
RegisterNetEvent('lapraces:client:StartRaceEditor')
AddEventHandler('lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        BJCore.Functions.Notify('You\'re already making a race', 'error')
    end 
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        BJCore.Functions.Notify('You have not placed a checkpoint', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["K"]) or IsDisabledControlJustPressed(0, Keys["K"]) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        BJCore.Functions.Notify('The minimum of checkpoints is 10', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["]"]) or IsDisabledControlJustPressed(0, Keys["]"]) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        BJCore.Functions.Notify('You cant go higher than 15')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["["]) or IsDisabledControlJustPressed(0, Keys["["]) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        BJCore.Functions.Notify('You cant go lower than 2')
                    end
                end
            else
                local coords = GetEntityCoords(PlayerPedId())
                BJCore.Functions.DrawText3D(coords.x, coords.y, coords.z, 'You have to be in a vehicle')
            end
            ...
            if IsControlJustPressed(0, Keys["9"]) or IsDisabledControlJustPressed(0, Keys["9"]) then
                if not CreatorData.ConfirmDelete then
                    ...
                    BJCore.Functions.Notify('Press [9] again to confirm', 'error', 5000)
                else
                    ...
                    BJCore.Functions.Notify('Race-editor Cancelled', 'error')
...
function SaveRace()
    ...
    BJCore.Functions.Notify('Race: '..CreatorData.RaceName..' is saved!', 'success')
...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            BJCore.Functions.Notify('You cant go too fast', 'error')
        end
    else
        BJCore.Functions.Notify('You cant go too fast', 'error')
    end
...
RegisterNetEvent('lapraces:client:UpdateRaceRacerData')
AddEventHandler('lapraces:client:UpdateRaceRacerData', function(RaceId, RaceData)
    ...
    while true do
        if RaceData.InCreator then
            ...
            if PlayerVeh ~= 0 then
                ...
                BJCore.Functions.DrawText3D(Offset.left.x, Offset.left.y, Offset.left.z, 'Checkpoint L')
                BJCore.Functions.DrawText3D(Offset.right.x, Offset.right.y, Offset.right.z, 'Checkpoint R')
            end
        else
            Citizen.Wait(500)
        end
...
RegisterNetEvent('lapraces:client:JoinRace')
AddEventHandler('lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        ...
        BJCore.Functions.Notify('You\'re already in a race', 'error')
    end
end)

RegisterNetEvent('lapraces:client:LeaveRace')
AddEventHandler('lapraces:client:LeaveRace', function(data)
    BJCore.Functions.Notify('You have left the race!')
    for k, v in pairs(CurrentRaceData.Checkpoints) do
...
RegisterNetEvent('lapraces:client:RaceCountdown')
AddEventHandler('lapraces:client:RaceCountdown', function()
    ...
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    BJCore.Functions.Notify('The race will start in 10 seconds', 'primary', 2500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    BJCore.Functions.Notify(Countdown, 'primary', 500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            ...
            BJCore.Functions.Notify('GO!', 'success', 1000)
        ...
        BJCore.Functions.Notify('You are not currently in a race', 'error')
    end
end)
    ...
    while true do
        ...
        if CurrentRaceData.RaceName ~= nil then
            if CurrentRaceData.Started then
                ...
                -- BJCore.Functions.DrawText3D(data.coords.x, data.coords.y, data.coords.z, 'Ga hier staan')
...
function FinishRace()
    ...
    if CurrentRaceData.BestLap ~= 0 then
        BJCore.Functions.Notify('Race finished: '..SecondsToClock(CurrentRaceData.TotalTime)..' | Best lap: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        BJCore.Functions.Notify('Race finished: '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
...
RegisterNetEvent('lapraces:client:PlayerFinishs')
AddEventHandler('lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherDataName)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            BJCore.Functions.Notify(FinisherDataName..' Finished on place : '..Place, 'error', 3500)
        end
    end
end)
...
RegisterNetEvent('lapraces:client:WaitingDistanceCheck')
AddEventHandler('lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    Citizen.CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            BJCore.Functions.Notify('Go back to the start in 10 seconds: '..ToFarCountdown..'s', 'error', 500)
                        else
