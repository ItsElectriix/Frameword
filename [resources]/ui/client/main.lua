...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedShooting(PlayerPedId()) then
            local StressChance = math.random(1, 8)
        ...
        if (PlayerJob.name ~= "police") then
            if IsPlayerFreeAiming(PlayerId()) and not IsPedShooting(PlayerPedId()) then
                ...
                local WeaponData = BJCore.Shared.Weapons[CurrentWeapon]
    ...
    while true do
        if not IsGaining then
            ...
            if StressGain > 0 then
                --BJCore.Functions.Notify('Stress increased', "primary", 2000)
                TriggerServerEvent('BJ-hud:Server:UpdateStress', StressGain)
                StressGain = 0
            end
        end
...
function GetShakeIntensity(stresslevel)
    local retval = 0.05
    for k, v in pairs(BJStress.Intensity["shake"]) do
...
function GetEffectInterval(stresslevel)
    local retval = 60000
    for k, v in pairs(BJStress.EffectInterval) do
        if stresslevel >= v.min and stresslevel < v.max then
            retval = v.timeout
            break
        end
...
Citizen.CreateThread(function()
    while true do
        ...
        if not isBlocked then 
            ...
            elseif stress >= BJStress.MinimumStress then
                local ShakeIntensity = GetShakeIntensity(stress)
                ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', ShakeIntensity)
