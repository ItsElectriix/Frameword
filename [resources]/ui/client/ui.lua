...
--     Citizen.Wait(500)
--     while true do 
--         if BJCore ~= nil and isLoggedIn and BJHud.Show then
    ...
    while true do
        if BJCore ~= nil and isLoggedIn and BJHud.Show then
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                speed = GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) * 3.6
                if speed >= BJStress.MinimumSpeed then
...
--     while true do
--         Citizen.Wait(1000)
--         if IsPedInAnyVehicle(PlayerPedId()) and isLoggedIn and BJHud.Show then
--             DisplayRadar(true)
--             SendNUIMessage({
--                 action = "car",
--                 show = true,
...
RegisterNetEvent('bj-hud:client:ToggleHarness')
AddEventHandler('bj-hud:client:ToggleHarness', function(toggle)
    SendNUIMessage({
        action = "harness",
        toggle = toggle
    })
...
RegisterNetEvent('bj-hud:client:UpdateNitrous')
AddEventHandler('bj-hud:client:UpdateNitrous', function(toggle, level, IsActive)
    SendNUIMessage({
        action = "nitrous",
        toggle = toggle,
        level = level,
...
RegisterNetEvent('bj-hud:client:UpdateDrivingMeters')
AddEventHandler('bj-hud:client:UpdateDrivingMeters', function(toggle, amount)
    SendNUIMessage({
...
RegisterNetEvent('bj-hud:client:UpdateVoiceProximity')
AddEventHandler('bj-hud:client:UpdateVoiceProximity', function(Proximity)
    SendNUIMessage({
        action = "proximity",
        prox = Proximity
    })
...
RegisterNetEvent('bj-hud:client:ProximityActive')
AddEventHandler('bj-hud:client:ProximityActive', function(active)
    SendNUIMessage({
        action = "talking",
        IsTalking = active
    })
...
Citizen.CreateThread(function()
    while true do
        if isLoggedIn and BJHud.Show and BJCore ~= nil then
            BJCore.Functions.TriggerServerCallback('hospital:GetPlayerBleeding', function(playerBleeding)
