                ...
                if seat == ped then
                    if not CheckDone then
                        if vehiclemeters == -1 then
                            CheckDone = true
                            QBCore.Functions.TriggerCallback('vehicletuning:server:IsVehicleOwned', function(IsOwned)
                                if IsOwned then
...
RegisterNetEvent('vehicletuning:client:UpdateDrivingDistance')
AddEventHandler('vehicletuning:client:UpdateDrivingDistance', function(amount, plate)
