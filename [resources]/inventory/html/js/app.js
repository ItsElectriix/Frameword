...
$(document).on("dblclick", ".item-slot", function(e){
    ...
    if ($(this).data("item") != null) {
        ...
        inv = $(this).parent().attr("data-inventory");
        if(itemData.useable) {
    ...
    switch(event.which) {
        case 3:
            ...
            if (CanQuickMove()) {
                ...
                if ($(fromInventory).attr('data-inventory').split("-")[0] == "itemsale" && toAmount == 0) {
                    InventoryError(fromInventory, fromSlot);
                    return;
                }
...
function FormatAttachmentInfo(data) {
    $.post("http://inventory/GetWeaponData", JSON.stringify({
        weapon: data.name,
        ...
        $(".weapon-attachments-container-details").html('<span style="font-weight: bold; letter-spacing: .1vh;">Serial Number</span><br> ' + ClickedItemData.info.serial + '<br><br><span style="font-weight: bold; letter-spacing: .1vh;">Durability - ' + Durability.toFixed() + '% </span> <div class="weapon-attachments-container-detail-durability"><div class="weapon-attachments-container-detail-durability-total"></div></div>')
        ...
        if (data.AttachmentData !== null && data.AttachmentData !== undefined) {
            if (data.AttachmentData.length > 0) {
                ...
                $.each(data.AttachmentData, function(i, attachment){
                    ...
                    $(".weapon-attachments").append('<div class="weapon-attachment" id="weapon-attachment-'+i+'"> <div class="weapon-attachment-label"><p>' + attachment.label + '</p></div> <div class="weapon-attachment-img"><img src="./images/' + attachment.item + '.png"></div> </div>')
                    attachment.id = i;
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post('http://inventory/RemoveAttachment', JSON.stringify({
...
$(document).on('click', '#weapon-attachments', function(e){
    e.preventDefault();
    if (!Inventory.IsWeaponAttachmentBlocked(ClickedItemData.name)) {
        ...
        $.post('http://inventory/Notify', JSON.stringify({
    ...
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
            ...
            if (itemData.info.attachments != null) {
                ...
                $(".item-info-description").html('<p><strong>Serial Number: </strong><span>' + itemData.info.serial + '</span></p><p><strong>Ammo: </strong><span>' + itemData.info.ammo + '</span></p><p><strong>Attachments: </strong><span>' + attachmentString + '</span></p>');
            } else{
                $(".item-info-description").html('<p><strong>Serial Number: </strong><span>' + itemData.info.serial + '</span></p><p><strong>Ammo: </strong><span>' + itemData.info.ammo + '</span></p><p>' + itemData.description + '</p>');
            }
        } else if (itemData.name == "filled_evidence_bag") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            if (itemData.info.type == "casing") {
                $(".item-info-description").html('<p><strong>Evidence material: </strong><span>' + itemData.info.label + '</span></p><p><strong>Type number: </strong><span>' + itemData.info.ammotype + '</span></p><p><strong>Caliber: </strong><span>' + itemData.info.ammolabel + '</span></p><p><strong>Serial Number: </strong><span>' + itemData.info.serial + '</span></p><p><strong>Crime scene: </strong><span>' + itemData.info.street + '</span></p><br /><p>' + itemData.description + '</p>');
                ...
                $(".item-info-description").html('<p><strong>Evidence material: </strong><span>' + itemData.info.label + '</span></p><p><strong>Fingerprint: </strong><span>' + itemData.info.fingerprint + '</span></p><p><strong>Crime Scene: </strong><span>' + itemData.info.street + '</span></p><br /><p>' + itemData.description + '</p>');
            ...
            }else if (itemData.info.type == "cash") {
                $(".item-info-description").html('<p><strong>Evidence material: </strong><span>' + itemData.info.label + '</span></p><p><strong>Amount: </strong><span>' + itemData.info.cash + '</span></p><br /><p>' + itemData.description + '</p>');
            }
        } else if (itemData.info.costs != undefined && itemData.info.costs != null) {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p>'+ itemData.info.costs + '</p>');
        } else if ((itemData.info.bank != undefined && itemData.info.bank != null) || (itemData.info.expires != undefined && itemData.info.expires != null)) {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            var description = '<p>' + itemData.description + '</p><br/>';
            if (itemData.info.bank != undefined && itemData.info.bank != null) {
                description = description + '<p><strong>Bank Ref: </strong><span>'+ itemData.info.bank + '</span></p>';
            }
            if (itemData.info.intel != undefined && itemData.info.intel != null) {
                description = description + '<p><strong>Intel: </strong><span>'+ itemData.info.intel + '</span></p>';
            }
            if (itemData.info.expires != undefined && itemData.info.expires != null) {
                description = description + '<p><strong>TS Ref: </strong><span>'+ itemData.info.expires + '</span></p>';
            }
            if (itemData.info.encrypted != undefined && itemData.info.encrypted != null) {
                description = description + '<p><strong>Encrypted: </strong><span>'+ itemData.info.encrypted + '</span></p>';
            }
        ...
        } else if (itemData.name == "washedstone") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p><strong>Ore: </strong><span>' + itemData.info.label + '</span></p>');
        } else if (itemData.name == "labkey") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p>Lab: ' + itemData.info.lab + '</p>');
        } else if (itemData.name == "white_winebottle" || itemData.name == "red_winebottle") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p>Type: ' + itemData.info.type + '</p>');
        } else if (itemData.name == "recorded_tape") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html(`<p>Title: ${itemData.info.title}</p><p>ID: ${itemData.info.id}</p>`);
        } else if (itemData.type == "cashstorage") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p><strong>Amount: </strong><span>$' + itemData.info.cash + '</span></p>');
        } else if (itemData.type == "clothing" && itemData.name != "shirt") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p><strong>Item: </strong><span>' + itemData.info.Prop + '</span></p><p><strong>Texture: </strong><span>' + itemData.info.Texture + '</span></p>');
        } else if (itemData.type == "clothing") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<strong>Shirt:</strong><p><strong>Item: </strong><span>' + itemData.info.shirt.Prop + '</span></p><p><strong>Texture: </strong><span>' + itemData.info.shirt.Texture + '</span></p>' +
                '<br><strong>Torso:</strong><p><strong>Item: </strong><span>' + itemData.info.torso.Prop + '</span></p><p><strong>Texture: </strong><span>' + itemData.info.torso.Texture + '</span></p>' +
                '<br><strong>Accessories:</strong><p><strong>Item: </strong><span>' + itemData.info.accessories.Prop + '</span></p><p><strong>Texture: </strong><span>' + itemData.info.accessories.Texture + '</span></p>');
        } else if (itemData.name == "receipt") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<strong>Employee: </strong><span>' + itemData.info.employee + '</span></p><p><strong>Amount: </strong><span>$' + itemData.info.amount + '</span></p>' +
                '<p><strong>Business: </strong><span>' + itemData.info.business + '</span></p>');
        // } else if (itemData.info.cooked != undefined && itemData.info.cooked != null && itemData.info.cooked == true) {
        //     $(".item-info-title").html('<p>'+itemData.label+'</p>')
        //     $(".item-info-description").html('<p>Cooked</p>');
        } else if (itemData.info.cooked != undefined && itemData.info.cooked != null && itemData.info.cooked == false) {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p>Not Cooked</p>');
        } else if (itemData.name == "cashcarrycard") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p>Job: ' + itemData.info.job + '</p>');
        } else {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
...
function handleDragDrop() {
    ...
    $("#item-give").droppable({
        ...
        drop: function(event, ui) {
            ...
            fromData = ui.draggable.data("item");
            toAmount = $("#item-amount").val();
            fromInv = ui.draggable.parent().attr("data-inventory");
            if (fromInv != "player" ) {
                InventoryError(ui.draggable.parent(), fromData.slot);
                return false
            }
...
function updateweights($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if ($fromInv.find("[data-slot=" + $fromSlot + "]").data("item").type == "itemstorage" && $toInv.attr("data-inventory").split("_")[0] == "stash-itemstorage") {
        InventoryError($fromInv, $fromSlot);
        return false;
    }

    if (($fromInv.attr("data-inventory").split("-")[0] == "itemshop" && $toInv.attr("data-inventory").split("-")[0] == "itemshop") || ($fromInv.attr("data-inventory") == "crafting" && $toInv.attr("data-inventory") == "crafting") || ($fromInv.attr("data-inventory") == "itemsale" && $toInv.attr("data-inventory") == "itemsale")) {
    ...
    if ($toAmount == 0 && $toInv.attr("data-inventory") == "itemsale") {
        itemData = $toInv.find("[data-slot=" + $toSlot + "]").data("item");
        if ($toInv.attr("data-inventory").split("-")[0] == "itemsale") {
...
//    if ($fromInv.attr("data-inventory").split("-")[0] == "itemsale") {
//        itemData = $fromInv.find("[data-slot=" + $fromSlot + "]").data("item");
//        if ($fromInv.attr("data-inventory").split("-")[0] == "itemsale") {
//            $fromInv.find("[data-slot=" + $fromSlot + "]").html('<div class="item-slot-img"><img src="images/' + itemData.image + '" alt="' + itemData.name + '" /></div><div class="item-slot-amount"><p>('+itemData.amount+') £'+itemData.price+'</p></div><div class="item-slot-label"><p>' + itemData.label + '</p></div>');
//        } else {
...
//    }
    ...
    if ($toInv.attr("data-inventory").split("-")[0] == "itemsale") {
        toItemData = $toInv.find("[data-slot=" + $toSlot + "]").data("item");
        fromItemData = $fromInv.find("[data-slot=" + $fromSlot + "]").data("item");
        if (toItemData.name != fromItemData.name) {
            if ($toInv.attr("data-inventory").split("-")[0] == "itemsale") {
                $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-img"><img src="images/' + toItemData.image + '" alt="' + toItemData.name + '" /></div><div class="item-slot-amount"><p>('+toItemData.amount+') £'+toItemData.price+'</p></div><div class="item-slot-label"><p>' + toItemData.label + '</p></div>');
            } else {
                $toInv.find("[data-slot=" + $toSlot + "]").html('<div class="item-slot-img"><img src="images/' + toItemData.image + '" alt="' + toItemData.name + '" /></div><div class="item-slot-amount"><p>'+toItemData.amount + ' (' + ((toItemData.weight * toItemData.amount) / 1000).toFixed(1) + ')</p></div><div class="item-slot-label"><p>' + toItemData.label + '</p></div>');
            }
...
$(document).on('click', '.CombineItem', function(e){
    e.preventDefault();
    if (combineslotData.toData.combinable.laptop === true) {
        $.post('http://inventory/combineLaptop', JSON.stringify({
            combineData: combineslotData.toData.combinable,
            usedItem: combineslotData.toData.name,
            usedItemSlot: combineslotData.fromData.slot,
            requiredItem: combineslotData.fromData.name,
            info: combineslotData.fromData.info,            
        }))
    } else if (combineslotData.toData.combinable.anim != null) {
        $.post('http://inventory/combineWithAnim', JSON.stringify({
            combineData: combineslotData.toData.combinable,
            usedItem: combineslotData.toData.name,
            requiredItem: combineslotData.fromData.name
        }))
    } else {
        $.post('http://inventory/combineItem', JSON.stringify({
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    ...
    $.post("http://inventory/SetInventoryData", JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {       
        ...
        //if ($fromInv.attr("data-inventory").split("-")[0] == "itemsale" && ($toInv.attr("data-inventory") == "player" || $toInv.attr("data-inventory") == "hotbar")) {
        //    InventoryError($fromInv, $fromSlot);
        //    return;
        //}
        ...
        if ($toAmount == 0 && $fromInv.attr("data-inventory").split("-")[0] == "itemshop" && $fromInv.attr("data-inventory") == "crafting") {
            InventoryError($fromInv, $fromSlot);
            return;
        } else if ($toAmount == 0 && $toInv.attr("data-inventory").split("-")[0] == "itemsale") {
            InventoryError($fromInv, $fromSlot);
            return;
        } else if ($toAmount == 0) {
        ...
        if((toData != undefined || toData != null) && toData.name == fromData.name && !fromData.unique) {
            ...
            $.post("http://inventory/PlayDropSound", JSON.stringify({}));
            $.post("http://inventory/SetInventoryData", JSON.stringify({
                fromInventory: $fromInv.attr("data-inventory"),
                toInventory: $toInv.attr("data-inventory"),
                fromSlot: $fromSlot,
            ...
            if (fromData.amount == $toAmount) {
                if (toData != undefined && toData.combinable != null && isItemAllowed(fromData.name, toData.combinable.accept) && $fromInv.attr("data-inventory") == "player" && $toInv.attr("data-inventory") == "player") {
                    $.post('http://inventory/getCombineItem', JSON.stringify({item: toData.combinable.reward}), function(item){
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("http://inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.IsWeaponAttachmentBlocked = function(WeaponName) {
        var BlockedWeapons = [ 
            "weapon_stungun", 
            "weapon_nightstick", 
            "weapon_flashlight",
        ...
        var retval = false;
        $.each(BlockedWeapons, function(i, name) {
            if (name == WeaponName) {
                retval = true;
            }
        });
    ...
    Inventory.Open = function(data) {
        ...
        if (data.other != null && data.other != "") {
            ...
            $(".other-inventory .item-slot").css({
                "background-color": "rgba(68, 90, 124, 0.05)"
            });
        }
        ...
        if (data.other != null) 
        {
            ...
            secondType = name.split("-")[0]
            $(".other-inventory").attr("data-type", secondType);
            if (name != null && (secondType == "itemshop" || secondType == "itemsale" || secondType == "crafting")) {
        ...
        if (secondType == 'crafting') {
            ...
            $('#item-use').show();
            $('#item-craft').hide();
            if (data.allowGive === true) {
                $('#item-give').show();
            }
            if (data.allowDestroy === true) {
                $('#item-destroy').show();
            }
        }
    ...
    Inventory.Close = function(ignorePost) {
        ...
        if (!ignorePost) {
            $.post("http://inventory/CloseInventory", JSON.stringify({}));
        }
...
$(document).on('click', '#rob-money', function(e){
    ...
    $.post('http://inventory/RobMoney', JSON.stringify({
        TargetId: TargetId
    }));
    $("#rob-money").remove();
});

function StripEmptyColumnsAndRows(data) {
    let rowInfo = [false, false, false];
    ...
    if (!rowInfo[0] && rowInfo[1] && !rowInfo[2]) {
        data.splice(1, 0, [false, false, false]);
    }
    if (data.length > 0) {
        for (var i = 2; i >= 0; i--) {
            let allEmpty = true;
...
function FindRecipe() {
    ...
    $.each($('.other-inventory .item-slot'), function(index, el) {
        var item = $(el).data('item') || {name:false};
        if (data[0].length < 3) {
            data[0].push(item.name);
        } else if (data[1].length < 3) {
            data[1].push(item.name);
        } else {
            data[2].push(item.name);
        }
    });
    
    data = StripEmptyColumnsAndRows(data);
    var match = false;
    for (key in Recipes) {
        var val = Recipes[key];
        var recipeOffset = 0;
        var allMatch = true;
        for (r in val.slotRecipe) {
            for (k in val.slotRecipe[r]) {
                let v = val.slotRecipe[r][k];
                if (!data[r] || data[r][k] != v) {
                    allMatch = false;
                }
            }
        }
...
function doCraftError() {
    $("#item-craft").css("background", "rgba(156, 20, 20, 0.5)").css("transition", "background 500ms");
    setTimeout(function() {
        $("#item-craft").css("background", "rgba(20, 20, 20, 0.5)");
    }, 500)
    $.post("http://inventory/PlayDropFail", JSON.stringify({}));
}

$(document).on('click', '#item-craft', function(e) {
    e.preventDefault();
    var craftedRecipe = FindRecipe();
  
    if (craftedRecipe && craftedRecipe != false) {
        console.log('Need to check requirement')
        $.post('http://inventory/CanCraft', JSON.stringify({
            recipe: craftedRecipe
        }), function(data) {
            console.log('Data returned: ' + data)
            if (data == 'OK') {
                Inventory.Close(true);
                $.post('http://inventory/DoCraft', JSON.stringify({
                    recipe: craftedRecipe
                }));
            } else {
                doCraftError();
