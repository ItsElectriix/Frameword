...
RegisterNetEvent('interior:client:SetNewState')
AddEventHandler('interior:client:SetNewState', function(bool)
	IsNew = bool
end)
...
function CreateHotelFurnished(spawn)
	...
	local POIOffsets = {}
	POIOffsets.exit = json.decode('{"z":2.344,"y":-3.62106,"x":-0.9079,"h":1.73957}')
	local model = GetHashKey("playerhouse_hotel")
    RequestModel(model)
	while not HasModelLoaded(model) do
	    Citizen.Wait(100)
	end
	local building = CreateObject(model, spawn.x - 0.7,spawn.y-0.4,spawn.z-1.42,false,false,false)
	...
	local mug2 = CreateObject(GetHashKey("V_Ret_TA_Mug"),spawn.x-2.2,spawn.y+0.9,spawn.z+0.9,false,false,false)
	local binder = CreateObject(GetHashKey("v_res_binder"),spawn.x-2.2,spawn.y+1.3,spawn.z+0.87,false,false,false)
	FreezeEntityPosition(motelstuff, true)
	FreezeEntityPosition(motelbed, true)	
	FreezeEntityPosition(motelclothes, true)
	FreezeEntityPosition(winframe, true)
	FreezeEntityPosition(glass, true)	
	FreezeEntityPosition(curtains, true)
	FreezeEntityPosition(screen, true)
	FreezeEntityPosition(trainer, true)	
	FreezeEntityPosition(trainer2, true)
