...
function takeHostage()
	...
	if closestPlayer ~= nil and closestPlayer ~= -1 then
		...
		for i=1, #hostageAllowedWeapons do
			if HasPedGotWeapon(PlayerPedId(), GetHashKey(hostageAllowedWeapons[i]), false) then
				if GetAmmoInPedWeapon(PlayerPedId(), GetHashKey(hostageAllowedWeapons[i])) > 0 then
					...
					BJCore.Functions.Notify("You have no ammo")
					return			
				end	
			else
				NoWeapon = true	
		...
		if NoWeapon then
			BJCore.Functions.Notify("You can\'t take someone hostage with this weapon")
			return
		end
		...
		if not holdingHostageInProgress then		
			...
			holdingHostage = true 
			TriggerServerEvent('cmg3_animations:sync', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag, false, 'start')
		end
	else
		BJCore.Functions.Notify("No one nearby")
	end
	...
	Citizen.CreateThread(function()
		while holdingHostage or beingHeldHostage do
			if holdingHostage then
				...
				BJCore.Functions.DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"[~g~G~s~] Release | [~r~H~s~] Kill")
...
function releaseHostage()
	...
	if closestPlayer ~= 0 then
		TriggerServerEvent('cmg3_animations:sync', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag, true, "release")
	end
end 
...
function killHostage()
	...
	target = GetPlayerServerId(closestPlayer)
	if target ~= 0 then
		TriggerServerEvent('cmg3_animations:sync', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag, true, "kill")
	end	
end 