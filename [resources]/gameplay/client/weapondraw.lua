	...
	while true do
		...
		if DoesEntityExist(ped) and not IsEntityDead(ped) then
			if currWeapon ~= GetSelectedPedWeapon(ped) then
				...
				if damageModifiers[newWeap] ~= nil then
					SetPlayerWeaponDamageModifier(PlayerId(), damageModifiers[newWeap])
				else
				...
				if CheckWeapon(newWeap) then
					if holstered then
						if BJCore.Functions.GetPlayerData().job.name == "police" then
