...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('http://phone/ClearRecentAlerts');
...
BJ.Phone.Functions.SetupRecentCalls = function(recentcalls) {
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    if  (RecentData && !RecentData.anonymous) {
        cData = {
                ...
                if (status.IsOnline) {
                    if (status.CanCall) {
                        if (!status.InCall) {
                            ...
                            BJ.Phone.Notifications.Add("fas fa-phone", "Phone", "You're already in a call");
                        }
                    } else {
                        BJ.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is busy");
                    }
                } else {
                    BJ.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is not available");
                }
            } else {
                BJ.Phone.Notifications.Add("fas fa-phone", "Phone", "You can't call yourself");
            }
        });
    } else {
        BJ.Phone.Notifications.Add("fas fa-phone", "Phone", "Unable to call anonymous number");
    }
});
...
BJ.Phone.Functions.LoadContacts = function(myContacts) {
    BJ.Phone.Contacts = myContacts;
    ...
    if (myContacts !== null) {
        ...
        $("#total-contacts").text("0 contacts Sadge");
    }
};
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== BJ.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            BJ.Phone.Functions.ToggleApp("phone", "none");
            BJ.Phone.Functions.ToggleApp("whatsapp", "block");
            BJ.Phone.Data.currentApplication = "whatsapp";
        
            $.post('http://phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
...
$(document).on('click', '#edit-contact', function(e){
    ...
    BJ.Phone.Animations.TopSlideDown(".phone-edit-contact", 200, 0);
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://phone/EditContact', JSON.stringify({
            ...
            BJ.Phone.Functions.LoadContacts(PhoneContacts);
        });
        BJ.Phone.Animations.TopSlideUp(".phone-edit-contact", 250, -100);
        setTimeout(function(){
            $(".phone-edit-contact-number").val("");
            $(".phone-edit-contact-name").val("");
        }, 250)
    } else {
        BJ.Phone.Notifications.Add("fas fa-exclamation-circle", "Edit Contact", "Fill out all fields");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('http://phone/DeleteContact', JSON.stringify({
        ...
        BJ.Phone.Functions.LoadContacts(PhoneContacts);
        BJ.Phone.Functions.LoadBankContactsWithNumber(PhoneContacts);
    });
    BJ.Phone.Animations.TopSlideUp(".phone-edit-contact", 250, -100);
    setTimeout(function(){
        $(".phone-edit-contact-number").val("");
...
$(document).on('click', '#phone-plus-icon', function(e){
    ...
    BJ.Phone.Animations.TopSlideDown(".phone-add-contact", 200, 0);
});
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        BJ.Phone.Animations.TopSlideUp(".phone-add-contact", 250, -100);
        setTimeout(function(){
        ...
        if (SelectedSuggestion !== null) {
            $.post('http://phone/RemoveSuggestion', JSON.stringify({
        ...
        BJ.Phone.Notifications.Add("fas fa-exclamation-circle", "Add Contact", "Fill out all fields");
    }
});
        ...
        if (cData.number !== BJ.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    BJ.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is in a call");
                }
            } else {
                BJ.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is not available");
            }
...
CancelOutgoingCall = function() {
    if (BJ.Phone.Data.currentApplication == "phone-call") {
        BJ.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
    ...
    if (!Canceled) {
        if (!BJ.Phone.Data.CallActive) {
            ...
            setTimeout(function(){
                ...
                $(".phone-app").css({"display":"none"});
                BJ.Phone.Functions.HeaderTextColor("white", 400);
                $("."+BJ.Phone.Data.currentApplication+"-app").css({"display":"none"});
...
BJ.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        CallData = cData;
...
$(document).on('click', '.phone-currentcall-container', function(e){
    ...
    BJ.Phone.Functions.HeaderTextColor("white", 500);
    BJ.Phone.Animations.TopSlideDown('.phone-application-container', 500, 0);
    BJ.Phone.Animations.TopSlideDown('.phone-call-app', 500, 0);
    BJ.Phone.Functions.ToggleApp("phone-call", "block");
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('http://phone/AnswerCall');
});

BJ.Phone.Functions.AnswerCall = function(CallData) {
    $(".phone-call-incoming").css({"display":"none"});
    $(".phone-call-outgoing").css({"display":"none"});
    $(".phone-call-ongoing").css({"display":"block"});
...
BJ.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
