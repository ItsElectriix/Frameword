...
SetupAppstore = function(data) {
    ...
    $.each(data.StoreApps, function(i, app){
        if (data.PhoneData.InstalledApps[i] == null || data.PhoneData.InstalledApps[i] == undefined) {
            var elem = '<div class="storeapp" id="app-'+i+'" data-app="'+i+'"><div class="storeapp-icon"><i class="'+app.icon+'"' + (app.color?` style="background-color: ${app.color};"`:'') + '></i></div><div class="storeapp-title">'+app.title+'</div> <div class="storeapp-creator">'+app.creator+'</div><div class="storeapp-download"><i class="fas fa-download"></i></div></div>'
            $(".store-apps").append(elem);
            app.app = i;
            $("#app-"+i).data('AppData', app);
        } else {
            var elem = '<div class="storeapp" id="app-'+i+'" data-app="'+i+'"><div class="storeapp-icon"><i class="'+app.icon+'"' + (app.color?` style="background-color: ${app.color};"`:'') + '></i></div><div class="storeapp-title">'+app.title+'<span style="font-size: 1vh;"> - Installed</span></div> <div class="storeapp-creator">'+app.creator+'</div><div class="storeapp-remove"><i class="fas fa-trash"></i></div></div>'
            $(".store-apps").append(elem);
            app.app = i;
            $("#app-"+i).data('AppData', app);
        }
...
function AppDownloadComplete(Installed) {
    if (Installed) {
        var applicationSlot = $(".phone-applications").find('[data-appslot="'+Installed.data.slot+'"]');
        var blockedapp = IsAppJobBlocked(Installed.data.blockedjobs, BJ.Phone.Data.PlayerJob.name)
        if ((!Installed.data.job || Installed.data.job === BJ.Phone.Data.PlayerJob.name) && !blockedapp) {
        ...
        BJ.Phone.Data.Applications[Installed.data.app] = Installed.data;
...
$(document).on('click', '.storeapp-download', function(e){
    ...
    if (AppData.password) {
        ...
        $(".download-progressbar-fill").animate({
            ...
            IsDownloading = false;
            $(".download-container").fadeOut(150, function(){
                $(".download-progressbar-fill").css("width", "0%");
            });

            $.post('http://phone/InstallApplication', JSON.stringify({
                app: CurrentApp,
            }), function(Installed){
                AppDownloadComplete(Installed);
            });
        });
    }
});
...
$(document).on('click', '.storeapp-remove', function(e){
    ...
    BJ.Phone.Data.Applications[AppData.app] = null;

    $.post('http://phone/RemoveApplication', JSON.stringify({
        app: AppData.app
    }));
    setTimeout(function(){
        $.post('http://phone/SetupStoreApps', JSON.stringify({}), function(data){
