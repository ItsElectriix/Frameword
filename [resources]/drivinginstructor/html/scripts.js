$(document).ready(function () {
  ...
  function setData(data, readOnly) {
    ...
    if (curTest.cid != undefined) {
      $("input[name='cid']").val(curTest.cid);
    } else {
      $("input[name='cid']").val('');
    }
  ...
  function closeGui() {
    if (readOnly) {
      // Don't record data
      $.post('http://drivinginstructor/close', JSON.stringify({ cid: -1, points: 10, passed: true, results: {} }));
    } else {
      curTest.results = {};
      ...
      $.post('http://drivinginstructor/close', JSON.stringify({ cid: curTest.cid, points: curTest.points, passed: curTest.passed, results: curTest.results }));
    }
  }
