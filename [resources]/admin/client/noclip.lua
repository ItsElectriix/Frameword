...
RegisterNetEvent('bj-admin:client:toggleNoclip')
AddEventHandler('bj-admin:client:toggleNoclip', function()
    toggleNoClipMode()
end)
...
function handleNoclip()
    ...
    function handleMovement(xVect,yVect)
        if ( IsControlJustPressed( 1, changeSpeedKey ) or IsDisabledControlJustPressed( 1, changeSpeedKey ) ) then
            ...
            BJCore.Functions.Notify("Current speed: ".. speeds[travelSpeed])

            updateForwardPush();
        end

        if ( IsControlJustPressed( 1, 36 ) or IsDisabledControlJustPressed( 1, 36 ) ) then
            travelSpeed = travelSpeed -1

            if ( travelSpeed < 1 ) then
                travelSpeed = getTableLength(speeds)
            end
