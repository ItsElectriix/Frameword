RegisterServerEvent('animalcrossing:server:banPlayer')
AddEventHandler('animalcrossing:server:banPlayer', function(reason, bsource)
    local src = source
    if src == nil or src == "" then src = bsource; end
    if GetConvar("server_type", "DEV") == "DEV" then print("[ADMIN] - Ban Triggered: "..reason.." | Player: "..GetPlayerName(src)) return; end
    TriggerEvent('animalcrossing:server:doBan', src, reason)
end)

AddEventHandler('animalcrossing:server:doBan', function(src, reason)
    local identifiers = BJCore.Functions.ExtractIdentifiers(src)
    TriggerEvent("bj-log:server:CreateLog", "anticheat", "AC Ban", "red", "**"..GetPlayerName(src) .. "** has been banned by AC for reason: "..reason.." | Player identifiers: "..BJCore.Common.Dump(identifiers))    
    ...
    DropPlayer(src, "You have been permanently banned by our automated system for cheating.")
end)
...
function NonRegisteredEventCalled(CalledEvent, source)
    ...
    if source ~= nil and source > 0 then
        TriggerEvent("animalcrossing:server:banPlayer", "(LUA Injection) Player tried to trigger event: "..CalledEvent, source)
    end
end
...
for k,v in pairs(ForbiddenEvents) do
    RegisterNetEvent(v)
...
AddEventHandler("giveWeaponEvent", function(sender, data)
    if data.givenAsPickup == false then
        TriggerEvent("bj-log:server:CreateLog", "anticheat", "Give Weapon Event", "red", "**"..GetPlayerName(sender).. "** ID: "..sender.." | **POSSIBLE** Attempt to give weapons to others players (networked). Data: "..BJCore.Common.Dump(data))  
        CancelEvent()
    end
end)
...
--         TriggerEvent("animalcrossing:server:banPlayer", "Attempt to clear ped tasks (networked)", source)
--     end
-- end)

local validResourceList
local function getValidRes()
    validResourceList = {}
...
getValidRes()
AddEventHandler("onResourceListRefresh", getValidRes)
RegisterNetEvent("chkRes")
AddEventHandler("chkRes", function(givenList)
    local src = source
    for _, resource in ipairs(givenList) do
        if not validResourceList[resource] then
            TriggerEvent("animalcrossing:server:banPlayer", "Executed client lua/resource - Client/Server resource mismatch", src)
            break
        end
    end
end)