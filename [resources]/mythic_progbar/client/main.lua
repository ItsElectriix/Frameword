...
function Progress(action, finish)
    ...
    if action.clearTasks == nil then
        mythic_action.clearTasks = true
    end
...
RegisterKeyMapping('-cancelprogress', 'Cancel Progress Bar~', 'keyboard', 'DELETE')
RegisterCommand('-cancelprogress', function()
    if not isDoingAction then return; end
    if not mythic_action.canCancel then return; end
    ...
    if not IsEntityDead(PlayerPedId()) or mythic_action.useWhileDead then
        if not isDoingAction then
            ...
            Citizen.CreateThread(function ()
                ...
                while isDoingAction do
                    Citizen.Wait(1)
                    if IsControlJustPressed(0, 121) and mythic_action.canCancel then -- INSERT KEY
                        TriggerEvent("mythic_progbar:client:cancel")
                    end
                end
                if finish ~= nil then
...
AddEventHandler("mythic_progbar:client:actionCleanup", function()
    ...
    if mythic_action.clearTasks then
    ...
    prop_net = nil
    DetachEntity(prop_ent, 1 ,1)
    DeleteEntity(prop_ent)
    prop_ent = nil
end)
