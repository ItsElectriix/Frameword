...
$(function() {
    $(document).on('click','#closeBtn, #close2',function(){
        $.post('https://coords/closeWindow', JSON.stringify({ 
        }));
    });
    ...
    $(document).on('click','#vector2',function(){
        copyToClipboard('vector2Copy');
    });
...
function updateCoords(x, y, z, h, json) {
    ...
    $('#vector2Copy').html("vector2("+parseFloat(x)+", "+parseFloat(y)+")")
