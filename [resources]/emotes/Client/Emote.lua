...
RegisterKeyMapping('+cancelemote', 'Hands Up/Cancel Emote~', 'keyboard', 'X')
  ...
  Citizen.CreateThread(function()
    while true do
      local ped = PlayerPedId()
			local sleep = 500
      if not IsPedInAnyVehicle(PlayerPedId(), true) and not isDead and not IsPedBeingStunned(ped, 0)then
...
AddEventHandler("openEmoteMenu", function() OpenEmoteMenu() end)
...
function EmotesOnCommand(source, args, raw)
  ...
  TriggerEvent('chatMessage', "SYSTEM", "warning", EmotesCommand)
...
function EmoteCommandStart(source, args, raw)
	...
	elseif DP.DogEmotes[name] ~= nil then
      if OnEmotePlay(DP.DogEmotes[name]) then end return
