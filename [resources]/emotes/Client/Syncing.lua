        ...
        if #args > 0 then
            ...
            if(distance ~= -1 and distance < 3) then
                if DP.Shared[emotename] ~= nil then
                    ...
                    SimpleNotify(Config.Languages[lang]['sentrequestto']..GetPlayerName(target).." ("..ename..")")
                else
...
AddEventHandler("ClientEmoteRequestReceive", function(emotename, etype)
    ...
    SimpleNotify(Config.Languages[lang]['doyouwanna']..remote..")")
end)

local function doSyncKeybind()
    Citizen.CreateThread(function()
        while true do
            ...
            if not IsPedInAnyVehicle(PlayerPedId(), true) then
                ...
                if not isRequestAnim then sleep = 1000; end
            end
            Citizen.Wait(sleep)
        end
    end)
...
function MearbysOnCommand(source, args, raw)
  ...
  TriggerEvent('chatMessage', "SYSTEM", "warning", NearbysCommand)
  --EmoteChatMessage(NearbysCommand)
  EmoteChatMessage(Config.Languages[lang]['emotemenucmd'])
end

function SimpleNotify(message)
    TriggerEvent('BJCore:Notify', message, 'inform', 8000)
end
