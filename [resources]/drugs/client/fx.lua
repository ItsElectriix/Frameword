...
InitPed = function()
  ...
  if isBadLSD then
    ...
    --TriggerEvent('InteractSound_CL:PlayOnOne', "clown", 0.55)
    SendNUIMessage({
      transactionType     = 'playSound',
      transactionFile     = "clown",
      transactionVolume   = 0.75
    })
  end
...
AddEventHandler('fx:toggleDrunkCamera', function(state)
    ...
    if state == "on" then
        ...
        BJCore.Functions.Notify("Drunk camera is now enabled")
    elseif state == "off" then
        enableDrunkCamera = false
        if isDrunk then
          ShakeGameplayCam('DRUNK_SHAKE', 0.0)
        end
        BJCore.Functions.Notify("Drunk camera is now disabled")
    else
        BJCore.Functions.Notify("You need to type in 'on' or 'off'")
    end
end)
