...
addEventListener("message", function(event){
    ...
    if(item.open == true) {
        ...
        if (item.class == "refresh-society") {
            $('#option-money').show();
            $('#society-amount').fadeIn();
            $('#society-amount').html(item.amount);
        }
        else if (item.class == "hide-society") {
            $('#society-amount').fadeOut();
            $('#option-money').hide();
        }

        if (item.class == "show-boss") {
            $('#option-employee').show();
            $('#option-recruit').show();
        }

        if (item.class == "hide-boss") {
            $('#option-employee').hide();
            $('#option-recruit').hide();
        }
...
$(document).on('click', '.fireemployee', function(e) {
    ...
    $.post('http://gangmenu/fireEmployee', JSON.stringify({
        source: playerid,
    }));
});
...
$(document).on('click', '.grade-box', function(e) {
    ...
    $.post('http://gangmenu/changeGrade', JSON.stringify({
        grade: gradeid,
...
$(document).on('click', '#option-recruit', function(e) {
    ...
    $.post('http://gangmenu/openRecruit', JSON.stringify({}));
});

$(document).on('click', '#option-bosstash', function(e) {

    $.post('http://gangmenu/openStash', JSON.stringify({}));
});

$(document).on('click', '#option-outfit', function(e) {

    $.post('http://gangmenu/outfit', JSON.stringify({}));
});
...
$(document).on('click', '.givejob', function(e) {
    ...
    $.post('http://gangmenu/giveJob', JSON.stringify({
        source: playerid,
    }));
});
...
$(document).on('click', '.deposit', function(e) {
    $.post('http://gangmenu/deposit', JSON.stringify({
        amount: currentDepositAmount,
    }));
});

$(document).on('click', '.withdraw', function(e) {
    $.post('http://gangmenu/withdraw', JSON.stringify({
