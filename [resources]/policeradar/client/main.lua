        ...
        if PlayerData.job.name == "police" then
            ...
            if (IsPedInAnyVehicle( ped ) ) then 
                ...
                if (IsPoliceVehicle(vehicle)) then
                    ...
                    BJCore.Functions.Notify( "You must be in a police vehicle", "error")
                end 
            else 
                BJCore.Functions.Notify( "You must be in a vehicle", "error")
            end 
        end
    end)
end)
...
AddEventHandler( 'wk:changeRadarLimit', function( speed ) 
    ...
    BJCore.Functions.Notify("Max speed set on: " .. speed .. "km/h")
end )
                        ...
                        if plateChecked ~= fwdPlate then
                            BJCore.Functions.TriggerServerCallback('police:IsPlateFlagged', function(result)
                                if result then
                                    ...
                                    radarInfo.fwdFastLocked = true 
                                    SendNUIMessage( { lockfwdfast = true } )
                                    BJCore.Functions.Notify("Flagged vehicle found (front)", "primary")
                                end
                        ...
                        if plateChecked ~= bwdPlate then
                            BJCore.Functions.TriggerServerCallback('police:IsPlateFlagged', function(result)
                                if result then
                                    ...
                                    radarInfo.bwdFastLocked = true 
                                    SendNUIMessage( { lockbwdfast = true } )
                                    BJCore.Functions.Notify("Flagged vehicle found (rear)", "primary")
                                end
        ...
        if BJCore ~= nil then 
            ...
            if (inVeh) then
                ...
                if (IsDisabledControlPressed(0, Keys["LEFTCTRL"]) and IsDisabledControlJustPressed(0, Keys["7"])) then 
                    if BJCore.Functions.GetPlayerData().job.name == "police" and BJCore.Functions.GetPlayerData().job.onduty then
                ...
                if (not IsDisabledControlPressed( 0, Keys["LEFTCTRL"]) and IsDisabledControlJustPressed( 0, Keys["7"])) then 
                    if BJCore.Functions.GetPlayerData().job.name == "police" and BJCore.Functions.GetPlayerData().job.onduty then
                        ...
                        ResetFrontAntenna()
                        ResetRearAntenna()
                        BJCore.Functions.Notify("Police ANPR has been reset")
                    end
                end 
                ...
                if (IsDisabledControlJustPressed( 0, Keys["8"])) then 
                    if BJCore.Functions.GetPlayerData().job.name == "police" and BJCore.Functions.GetPlayerData().job.onduty then
                        ...
                        if (radarInfo.plateLocked) then
                            ...
                            BJCore.Functions.Notify("License plate locked: " .. radarInfo.fwdPlate)
                        else
                            SendNUIMessage({ unlockPlate = true })
                            BJCore.Functions.Notify("License plate unlocked")
                        end
                    end
                end
            end                
