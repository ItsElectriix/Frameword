...
RegisterServerEvent('bj-scoreboard:server:playerJoin')
AddEventHandler("bj-scoreboard:server:playerJoin", function()
    ...
    TriggerClientEvent("bj-scoreboard:client:AddPlayer", -1, data )
    ST.Scoreboard.AddAllPlayers()
end)

function ST.Scoreboard.AddAllPlayers(self)
    local players
    players = exports['voice']:GetInfinityPlayers()
    ...
    for k,v in pairs(players) do
        ...
        local identifiers, steamIdentifier = GetPlayerIdentifiers(k)
        for _, v in pairs(identifiers) do
            if string.find(v, "steam") then
                steamIdentifier = v
                break
        ...
        local stid = HexIdToSteamId(steamIdentifier)
        local scomid = steamIdentifier:gsub("steam:", "")
        data[k] = { src = k, steamid = stid, comid = scomid, name = v.name }
    end
	TriggerClientEvent("bj-scoreboard:client:AddAllPlayers", source, data)
end
...
AddEventHandler("playerDropped", function()
    ...
    TriggerClientEvent("bj-scoreboard:client:RemovePlayer", -1, data)
    Wait(600000)
    TriggerClientEvent("bj-scoreboard:client:RemoveRecent", -1, plyid)
end)
